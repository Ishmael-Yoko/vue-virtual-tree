<template>
  <div class="virtual-tree" ref="scroller" @scroll="handleScroll">
    ...
    >
      <template v-for="(item, index) in visibleData">
        <div
          :key="item[option.primaryKey || 'id']"
          @click="nodeClick(item)"
          :style="{ height: option.itemHeight + 'px', 'line-height': option.itemHeight + 'px', paddingLeft: !option.left ? 18 * (item.level - 1) + 'px' : (20 + 18 * (item.level - 1)) + 'px', ...(item[option.primaryKey || 'id'] === currentData[option.primaryKey || 'id'] ? option.activeStyle : {}) }"
          :class="{
            'virtual-tree-node': true,
            'is-current': item[option.primaryKey || 'id'] === currentData[option.primaryKey || 'id']
          }"
        >
          <i
            :style="{ 'line-height': option.itemHeight + 'px', color: iconColor, 'margin-right': option.iconGap + 'px' }"
            :class="{
...
export default {
  ...
  props: {
    ...
    option: {
      type: Object,
      required: false,
      default: () => ({ itemHeight: 25, key: 'id', left: 0, iconGap: 0, activeStyle: {} })
    },
    lazy: {
      type: Boolean,
      default: false
  ...
  computed: {
    visibleCount () {
      const height = this.height || this.$refs.scroller.offsetHeight
  ...
  methods: {
    ...
    // 处理下拉
    handleExpand () {
      const filterNode = this.flattenTree().filter(c => this.expandKeys.find(k => k === c[this.option.primaryKey || 'id']))
    ...
    setCurrent (current) {
      if (current !== undefined) {
        this.currentData = typeof current === 'object' ? current : { [this.option.primaryKey || 'id']: current }
      }
      this.updateView()
    },
    // 是否显示expand
          ...
          if (node.children && node.children.length) {
            ...
            this.load(node)
              .then(data => {
                ...
                this.expand(node, true)
                this.updateView()
                const expands = data.filter(l =>
                  this.expandKeys.find(m => l[this.option.primaryKey || 'id'] === m)
                )
                if (expands.length) {
                  expands.forEach(j => {
                    this.toggleExpand(false, j, {})
                  })
                } else {
                  this.$emit('loaded')
                }
    ...
    setCheckKeys (keys) {
      this.flattenTree().forEach(c => {
        const node = keys.find(m => c[this.option.primaryKey || 'id'] === m)
        if (node) {
          c.checked = true
        } else {
          c.checked = false
    ...
    handleParentChange (check, node) {
      const recursionCheck = parent => {
        ...
        const otherCheck =
          checkLength.length > 0 && checkLength.length < parent.children.length
        ...
        if (
          parent.parent &&
          parent[this.option.primaryKey || 'id'] &&
          parent.level !== 0 &&
      ...
      if (
        node.parent &&
        node[this.option.primaryKey || 'id'] &&
        node.level !== 0 &&
    ...
    filter (val) {
      const recursionCheck = (arr, parent) => {
        ...
        if (
          ...
          parent.children.some(c => c.visible)
        ) {
          parent.visible = true
        }
      }
...
.virtual-tree-node .el-checkbox {
  margin-right: 8px !important;
}
.virtual-tree-node.is-current {
  background-color: #F4F4F4;
}
.virtual-tree-node .virtual-tree-node__loading-icon {
  width: 15px;
  height: 15px;
  line-height: 15px;
  text-align: center;
...
.virtual-tree-node > i.is_expand {
  transform: rotateZ(90deg);
}
.virtual-tree-node > i.is_leaf {
